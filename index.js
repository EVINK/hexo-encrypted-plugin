...
function genNewSalt(title, iv) {
    ...
    let fileName = folderName + 'encrypted.json'
    if (iv) fileName = folderName + 'encrypted2.json'
    let data = {}
    if (fs.existsSync(fileName)) {
        data = fs.readFileSync(fileName).toString()
        data = JSON.parse(data)
    }
    const value = genRandomString(32)
    data[title] = value
...
function genAES() {
    ...
    const fileName = folderName + 'aes.js'
    if (!fs.existsSync(fileName)) {
        data = fs.writeFileSync(fileName, _genAES())
    }
}
...
hexo.extend.filter.register('after_post_render', (p) => {
    ...
    // 确定model的值
    if (p.encryptedModel) {
        const idx = p.encryptedModel.indexOf('/')
        if (idx > 0) model = p.encryptedModel.slice(0, idx)
        else if (idx === 0) {
            model = p.encryptedModel.slice(1)
            const idx = model.indexOf('/')
            if (idx > 0) model = model.slice(0, idx)
        } else model = p.encryptedModel
    }

    if (!model) model = '@encrypted'
    const _model_exists = pages.includes(model)
    const _dir = 'source/' + model
    if (!_model_exists) {
        const _dirExsist = fs.existsSync(_dir)
        if (!_dirExsist) fs.mkdirSync(_dir)
    }
    ...
    p.content = aesEle + data + `<div class="encrypted-plug-div" title="${p.title}" style="display:none">
    ...
    <div class="auth-wrapper">
        ...
        <svg class="unlock-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
            <g fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <rect x="5" y="11" width="14" height="10" rx="2"></rect>
                <circle cx="12" cy="16" r="1"></circle>
                <path d="M8 11V6a4 4 0 0 1 8 0"></path>
            </g>
        </svg>
    </div>
</div>
